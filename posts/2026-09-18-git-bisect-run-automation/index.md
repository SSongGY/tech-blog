...
"지난주엔 분명 됐는데 지금은 안 된다"는 제보가 들어온다. 그 사이 커밋은 60개가 넘고, 배포 로그를 봐도 범인이 특정되지 않는다. 이때 대부분은 최근 커밋부터 눈으로 훑기 시작하는데, 이건 최악의 경우 60번을 봐야 하는 선형 탐색이다.

`git bisect`는 이 문제를 이분 탐색으로 바꾼다. 그리고 `git bisect run`을 쓰면 사람이 한 번도 개입하지 않고 끝난다. 이 글은 커밋 64개짜리 저장소를 실제로 만들어 돌려보고, 대부분의 설명이 건너뛰는 부분 — **빌드조차 안 되는 커밋을 만났을 때** — 까지 확인한다.
...
```mermaid
flowchart LR
    subgraph range["커밋 64개 (r000 ~ r063)"]
        direction LR
        g["r000<br/>good (기준)"]
        mid["r031<br/>?"]
        b["r063<br/>bad (현재)"]
    ...
    mid -->|"테스트 통과 → good"| right["탐색 범위<br/>r032 ~ r063"]
    mid -->|"테스트 실패 → bad"| left["탐색 범위<br/>r000 ~ r031"]
...
```mermaid
flowchart TD
    run["테스트 명령 실행"] --> code{"종료 코드"}
    code -->|"0"| good["good — 범위를 뒤쪽으로"]
    code -->|"1 ~ 124, 126, 127"| bad["bad — 범위를 앞쪽으로"]
    code -->|"125"| skip["skip — 판정 불가, 이웃 커밋으로"]
    code -->|"128 이상"| abort["bisect 즉시 중단"]

    style good fill:#2d6a4f,color:#fff
    style bad fill:#9d0208,color:#fff
    style skip fill:#9c6644,color:#fff
```
