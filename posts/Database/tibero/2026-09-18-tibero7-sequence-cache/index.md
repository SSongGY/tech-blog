...
description: "Tibero 7.2에서 시퀀스 예제를 직접 돌려 확인했다. 기본 MAXVALUE는 INT64_MAX가 아니라 9가 28개이고 내림차순 MINVALUE는 27개로 한 자리 비대칭이다. 기본 캐시는 20개. ALTER SEQUENCE는 매뉴얼과 달리 번호를 건너뛰지 않았고, 캐시는 세션이 아니라 인스턴스에 붙어 있다."
difficulty: 입문
product: Tibero
product_version: "7"
feature: sequence
...
> (`ALTER SEQUENCE`가 캐시에 존재하던 값을 무효화한다는 점).
> 캐시의 내부 자료구조나 갱신 시점의 락 동작은 매뉴얼에 근거가 없어 그리지 않았다.
> 오른쪽 초록 칸의 `ALTER SEQUENCE` 동작은 매뉴얼이 아니라 **Tibero 7.2에서 직접
> 돌려 확인한 값**이다. 아래 「ALTER SEQUENCE는 번호를 건너뛰지 않았다」 절에 있다.
...
매뉴얼이 드는 것은 **세 가지**인데, 이 환경에서 확인해 보니 그중 하나는 실제로
번호를 건너뛰지 않았다. 재현할 수 있는 것과 없는 것을 나눠 적는다.

1. **비정상 종료.** 캐시에 저장된 값이 유실될 수 있다. `CACHE N`이면 최대 N-1개가 빈다.
   인스턴스를 강제로 내려야 재현되는 경로라 **이 환경에서는 확인하지 않았다** —
   공유 서버를 죽일 수 없다. 매뉴얼 근거로만 적는다.
2. **클러스터 환경의 `NOORDER`.** 노드별 캐시를 유지하므로 노드 안에서는 순서가
   지켜지지만 **전체 노드 기준 순서는 달라진다.** 순서를 맞춰야 하면 `ORDER`를 쓰는데,
   캐시로 얻던 이점을 포기하는 선택이 된다. 단일 인스턴스라 **확인할 수 없었다.**
3. **`ALTER SEQUENCE`.** 매뉴얼은 캐시에 있던 값을 무효화해 일부 값이 누락될 수
   있다고 적는다. **돌려 보니 7.2에서는 누락되지 않았다.** 다음 절에 측정값이 있다.
...
`CACHE 5`로 한 칸씩 확인하면 갱신 시점이 보인다. `NEXTVAL`이 1에서 5로 가는 동안
`LAST_NUMBER`는 6에서 움직이지 않다가, 캐시가 바닥난 여섯 번째에 11로 뛴다.
**`LAST_NUMBER`가 멈춰 있는 구간이 곧 캐시가 일하는 구간이다.**

### ALTER SEQUENCE는 번호를 건너뛰지 않았다

매뉴얼은 `ALTER SEQUENCE`가 캐시에 있던 값을 무효화해 일부 값이 누락될 수 있다고 적는다.
그렇다면 `CACHE 20`인 시퀀스에서 1을 뽑아 2~20이 캐시에 남은 상태로 `ALTER`를 걸면
다음 값은 21이어야 한다. 네 가지 `ALTER`로 확인했다.

| 건 것 | ALTER 전 | ALTER 후 | LAST_NUMBER |
|---|---|---|---|
| `CACHE 20` (같은 값) | 1 | **2** | 21 → 22 |
| `INCREMENT BY 5` | 1 | **6** | → 106 |
| `CACHE 30` | 1 | **2** | → 32 |
| `NOCACHE` | 1 | **2** | → 3 |

**네 경우 모두 21이 아니라 2에서 이어졌다.** `INCREMENT BY 5` 쪽이 6인 것도 1에 5를
더한 값이니 같은 이야기다. 즉 `ALTER`는 캐시를 버리되 **아직 발급하지 않은 값을
되돌려 놓는다.** 마지막 발급값 다음부터 다시 확보하므로 번호가 비지 않는다.

세션을 넘어가도 마찬가지였다. 캐시를 채운 세션을 끝내고 새 세션에서 뽑아도,
또 다른 세션에서 `ALTER`를 걸어도 번호는 이어졌다.

```text
세션 1: NEXTVAL → 1,  LAST_NUMBER 21   (여기서 세션 종료)
세션 2: NEXTVAL → 2,  LAST_NUMBER 21
세션 3: ALTER 뒤 NEXTVAL → 3,  LAST_NUMBER 21 → 23
```

세션 2가 21이 아니라 2를 받았다는 것은 **캐시가 세션이 아니라 인스턴스에 붙어 있다**는
뜻이다. 접속을 끊는다고 잡아 둔 번호가 날아가지는 않는다.

이 결과가 **매뉴얼이 틀렸다는 뜻은 아니다.** 무효화 자체는 일어난다 — `ALTER` 직후
`LAST_NUMBER`가 다시 잡히는 것이 그 흔적이다. 다만 7.2에서는 그것이 **번호 누락으로
이어지지 않았다.** 버전에 따라 달라질 수 있으니 연속성이 중요하면 쓰는 버전에서
직접 확인하는 편이 맞다.

> 수행 기록: [`code/output.txt`](code/output.txt),
> 세션을 나눠 돌린 쪽은 [`code/cache_cross_session.sql`](code/cache_cross_session.sql)과
> [`code/output-cross-session.txt`](code/output-cross-session.txt)에 있다.

## 실무에서 주의할 점

- **연속성이 필요하면 시퀀스를 쓰지 않는다.** 세금계산서 번호처럼 빈 번호가 문제면
  맞는 도구가 아니다. 비정상 종료 한 번이면 구멍이 생긴다.
- **`CACHE` 개수는 발급 속도와 최대 누락 개수를 동시에 정한다.** 크게 잡으면 데이터
  사전 갱신이 줄지만 비정상 종료 시 잃는 번호도 그만큼 늘어난다.
- **`ALTER SEQUENCE`가 번호 구멍의 원인이라고 먼저 의심하지 않는다.** 7.2에서 네 가지
  `ALTER`를 걸어 봤지만 번호는 모두 이어졌다. 구멍이 실제로 있다면 비정상 종료나
  클러스터 쪽을 먼저 본다. 다만 버전에 따라 달라질 수 있으니 **쓰는 버전에서 한 번
  확인하고 넘어간다.** 확인 방법은 이 글의 예제 그대로다.
...
- 매뉴얼이 든 경로 셋 중 **`ALTER SEQUENCE`는 7.2에서 번호를 건너뛰지 않았다.**
  네 가지 `ALTER` 모두 마지막 발급값 다음에서 이어졌다. 남은 둘(비정상 종료,
  클러스터 `NOORDER`)은 이 환경에서 재현할 수 없어 매뉴얼 근거로만 적었다.
- 캐시는 세션이 아니라 **인스턴스**에 붙어 있다. 세션을 끊어도 잡아 둔 번호는 남는다.
