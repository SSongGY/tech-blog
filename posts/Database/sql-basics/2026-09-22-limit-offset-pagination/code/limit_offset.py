...
def main():
    ...
    for label, sql in (
        ...
        print(f"   {label}: " + " / ".join(step[3] for step in plan))
    print()
    ...
    for label in ("인덱스 없음", "인덱스 생성 후"):
        ...
        plan = conn.execute("EXPLAIN QUERY PLAN SELECT id FROM article LIMIT 3")
        detail = " / ".join(step[3] for step in plan.fetchall())
        print(f"   {label:<12} 결과 {[r[0] for r in rows]}")
        print(f"   {'':<12} 계획 {detail}")
    print()
