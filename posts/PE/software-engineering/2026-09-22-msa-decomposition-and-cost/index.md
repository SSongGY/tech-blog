...
**분해 기준은 설계 드라이버에 들어 있다.** NIST SP 800-204의 설계 드라이버 절(§2.2)은 드라이버를 **4가지**로 든다.
...
여기서 **2번이 분해 기준의 본문**이다. "단일 기능 + 경계 컨텍스트". 여기에 장점 절(§2.10)이 덧붙이는
...
> API 게이트웨이가 인증·라우팅·유량 제어를 묶어 제공한다는 것은 같은 문서의 게이트웨이 절(§2.6~§2.7.1)에서 가져왔다.
...
앞의 네 줄은 NIST SP 800-204 Table 3의 내용이고, 뒤의 두 줄은 같은 문서의 다른 두 절(§2.4·§2.11)에서 온다.

**SOA와의 관계도 자주 묻는다.** 같은 문서의 SOA 비교 절(§2.9)은 서비스·상호운용성·느슨한 결합이라는
...
NIST SP 800-204의 단점 절(§2.11)이 드는 **6가지**가 그대로 고려사항이 된다.
...
- **분해 기준 3개**: 단일 기능 · 경계 컨텍스트 · 업무 능력과의 정렬.
  앞의 둘은 설계 드라이버 절(§2.2), 마지막은 장점 절(§2.10)에서 온다.
...
- [NIST SP 800-190, Application Container Security Guide (2017-09)](https://csrc.nist.gov/pubs/sp/800/190/final) — SP 800-204(§1.3)가 마이크로서비스를 컨테이너로 구현할 때 함께 볼 문서로 지목한 지침
