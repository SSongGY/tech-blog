...
1. 물류·가공·검사까지 **한 사이클이 사람의 개입 없이 수행된다**
...
2. **자율 조치** — 예측에서 그치지 않고 조치까지 자동으로 이뤄져야 한다 (acatech 6단계).
...
- **무인화를 막는 것은 대개 정상 경로가 아니라 예외다.** 가공·이송·검사의 정상 흐름은
  ...
  마모됐을 때처럼 빈도는 낮고 종류는 많은 상황이다. 이런 예외를 얼마나 없앴느냐가
...
- **공정 단위로 끊어서 넓힌다.** 공장 전체를 한 번에 무인화하는 대신 변동이 작은
  공정부터 무인으로 돌리고, 그 구간의 무인 가동 시간을 지표로 잡아 넓힌다.
...
- 정의는 두 요건으로 끊는다 — **한 사이클이 사람 개입 없이 수행된다 + 사람은 원격에만 있다.**
...
- 다크 팩토리의 성립선은 **5단계와 6단계 사이**다. 5단계는 예측하고 사람이 조치하지만,
  6단계는 조치까지 설비가 스스로 수행한다.
- 무인화를 막는 것은 설비가 아니라 **드물게 일어나는 예외**와 **어디까지 기계에 맡길지**다.
