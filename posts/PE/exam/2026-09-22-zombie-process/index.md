...
**좀비 프로세스(Zombie Process)** 는 POSIX.1-2024 §3.426이 **실행이 종료되었으나
그 프로세스 ID가 아직 시스템에 반납되지 않은 프로세스**로 정의한다. 같은 표준
§3.285는 프로세스 수명을 생성 시점부터 PID가 반납되는 시점까지로 정하므로,
좀비는 **실행이 끝난 뒤에도 수명이 이어지고 있는 구간**이다.
